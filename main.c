...
int jmlitem;
tabel tabel1[10],tabel2[10],tabel3[10],tabel4[10];

void main()
{
    printf("Masukan berapa item : ");
    scanf("%d",&jmlitem);
    int j,k[jmlitem],l[jmlitem],m[jmlitem],n[jmlitem];
    int acc=0;
    for(j=0; j<jmlitem; j++)
    {
        printf("Masukkan nilai w%d =",j+1);
        scanf("%d",&tabel1[j].w);
        printf("Masukkan nilai p%d =",j+1);
        scanf("%d",&tabel1[j].p);
        tabel1[j].d=tabel1[j].p/tabel1[j].w;
        tabel1[j].i=j+1;
    }
    printf("Kapasitas Knapsack W:");
    scanf("%d",&knapsack);
